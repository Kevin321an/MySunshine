     ...
     This method can be called from multiple threads, as described in Processes and Threads.
        ...
        switch (match) {
            case WEATHER: {
                ...
                if (_id > 0)
                    returnUri = WeatherContract.WeatherEntry.buildWeatherUri(_id);//build the content Uri follow by ID
                else
            ...
            case LOCATION:{
                ...
                if(_id>0)
                    returnUri=WeatherContract.LocationEntry.buildLocationUri(_id);//build the content Uri follow by ID
                else
                    throw new android.database.SQLException("Failed to insert row into " + uri);
                break;
            }
